      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="bg-red-100 px-3 py-1 rounded-full">
              <span className="text-red-800 text-xs font-medium">Secure View Only</span>
            </div>
            <button 
              onClick={onClose}
        ...
        <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center text-xs text-gray-500">
            ...
            This document is protected. Downloads, screenshots, copying, and printing are disabled.
          </div>
        </div>
      </div>
    </div>
...
const OTPVerification = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:3000/api/session-status?patientUsername=${sessionInfo.patientUsername}`);
  ...
  const handleFileClick = async (filename) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/api/get-file-content`, {
        params: {
      ...
      if (response.data.success) {
        ...
        alert("Error: " + (response.data.message || "Unable to load file content."));
      }
    } catch (error) {
      console.error("Error fetching file content:", error);
      alert("Error: Unable to load file content. " + (error.response?.data?.message || "Please try again."));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-b from-blue-50 to-blue-100">
      ...
      <SecureFileModal 
        ...
        content={fileContent}
        isPdf={selectedFile ? (typeof selectedFile === 'object' ? selectedFile.isPdf : isPdfFile(selectedFile)) : false}
      />
                        ...
                        >
                          <div className="flex items-center">
                            <div className="mr-3 bg-blue-100 p-2 rounded">
                              <svg className={`h-6 w-6 ${isPdfFile(filename) ? 'text-red-600' : 'text-blue-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                {isPdfFile(filename) ? (
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">Click to view in secure reader</p>
                            </div>
                          </div>
                        </div>
                      ))
